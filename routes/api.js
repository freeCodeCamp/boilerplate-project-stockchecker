  ...
  app.route("/api/stock-prices").get(function (req, res) {
    ...
    async function getStockData(stock, like, reqIP) {
      ...
      if (stock) {
        if (like === "true") {
          if (stocks.length > 1) {
            let stockData1 = await updateOrCreateNewStock(
              stocks[0],
              like,
              reqIP
            );

            let stockData2 = await updateOrCreateNewStock(
              stocks[1],
              like,
              reqIP
            );
          ...
          if (stocks.length > 1) {
            ...
            await updateOrCreateNewStock(stocks[0], like, reqIP).then(
              (data) => {
                if (data.stock && data.price) {
                  res.json({
                    stockData: {
    ...
    console.log("provided stock:", stock);
